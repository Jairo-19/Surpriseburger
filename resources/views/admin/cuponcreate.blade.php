@extends('pagina.layouts.plantilla_admin')
@section('title', 'Creación de cupones | Surprise Burger')

@section('content')

<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100 p-4 sm:p-6 lg:p-8">

    <div class="w-full max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden">

        <!-- Alertas -->
        @include('components.alert')

        <!-- Header -->
        <div class="bg-[#1E3D9A] px-8 py-10 md:px-12 md:py-12">
            <h1 class="text-3xl md:text-4xl lg:text-5xl text-white text-center mb-2">
                @if(isset($cupon)) Editar Cupón @else Crear Cupón @endif
            </h1>
            <p class="text-blue-100 text-center text-sm md:text-base">
                Gestiona los cupones de recompensas
            </p>
        </div>

        <!-- FORMULARIO -->
        <form 
            action="{{ isset($cupon) ? route('admin_cupones.update', $cupon->id) : route('admin_cupones.store') }}" 
            method="POST" 
            enctype="multipart/form-data"
            class="px-8 py-10 md:px-12 md:py-12 space-y-8"
        >
            @csrf
            @if(isset($cupon))
                @method('PUT')
            @endif

            <!-- Nombre -->
            <div class="group">
                <label class="block mb-3 text-sm font-bold text-gray-700 flex items-center gap-2">
                    <i class="bi bi-card-text text-[#1E3D9A]"></i>
                    Nombre del cupón
                </label>
                <input
                    type="text"
                    name="nombre"
                    required
                    placeholder="10% descuento"
                    class="w-full border-2 border-gray-300 rounded-xl px-5 py-4 focus:outline-none focus:border-[#1E3D9A] focus:ring-4 focus:ring-blue-100 @error('nombre') border-red-500 @enderror"
                    value="{{ old('nombre', $cupon->nombre ?? '') }}"
                />
                @error('nombre')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>

            <!-- Puntos necesarios -->
            <div class="group">
                <label class="block mb-3 text-sm font-bold text-gray-700 flex items-center gap-2">
                    <i class="bi bi-gift-fill text-[#1E3D9A]"></i>
                    Puntos necesarios
                </label>
                <input
                    type="number"
                    name="puntos_necesarios"
                    min="0"
                    required
                    placeholder="500"
                    class="w-full border-2 border-gray-300 rounded-xl px-5 py-4 focus:outline-none focus:border-[#1E3D9A] focus:ring-4 focus:ring-blue-100 @error('puntos_necesarios') border-red-500 @enderror"
                    value="{{ old('puntos_necesarios', $cupon->puntos_necesarios ?? '') }}"
                />
                @error('puntos_necesarios')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>

            <!-- Imagen -->
            <div class="group">
                <label class="block mb-3 text-sm font-bold text-gray-700 flex items-center gap-2">
                    <i class="bi bi-image-fill text-[#1E3D9A]"></i>
                    Imagen del cupón
                </label>

                {{-- Vista previa de la imagen actual --}}
                @if(isset($cupon) && $cupon->imagenes)
                    <div class="mb-4">
                        <p class="text-xs text-gray-500 mb-2">Imagen actual:</p>
                        <img 
                            src="{{ asset('storage/' . $cupon->imagenes) }}" 
                            alt="{{ $cupon->nombre }}" 
                            class="w-32 h-32 object-cover rounded-xl shadow-md border-2 border-gray-100"
                        >
                    </div>
                @endif
                <input
                    type="file"
                    name="imagen"
                    accept="image/*"
                    class="w-full border-2 border-gray-300 rounded-xl px-5 py-4"
                />
                @error('imagen')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>

            <!-- Botón -->
            <button
                type="submit"
                class="w-full bg-[#1E3D9A] text-white py-5 rounded-xl text-lg font-bold hover:bg-[#152d73] transition shadow-lg flex items-center justify-center gap-3"
            >
                <i class="bi bi-check-circle-fill"></i>
                @if(isset($cupon)) Actualizar cupón @else Crear cupón @endif
            </button>

        </form>
    </div>
</div>

@endsection
